...
import type { Profile, Subject, Lesson, HomeFeaturedSubject, HomeFeaturedLesson } from '@/types/database';
...
export default function HomepageManagement() {
    ...
    const moveSubject = async (item: any, direction: 'up' | 'down') => {
        ...
        await supabase.from('home_featured_subjects').update({ home_order: newIndex }).eq('id', item.id);
        await supabase.from('home_featured_subjects').update({ home_order: currentIndex }).eq('id', otherItem.id);
    ...
    const addFeaturedSubject = async () => {
        ...
        const maxOrder = featuredSubjects.length > 0
            ? Math.max(...featuredSubjects.map(s => s.home_order)) + 1
            : 0;

        const { error } = await supabase
            .from('home_featured_subjects')
            .insert({ subject_id: selectedItemId, home_order: maxOrder } as any);

        if (error) {
            if (error.code === '23505') {
                toast.error(t('هذه المادة مضافة مسبقاً', 'Subject already featured'));
            } else {
                toast.error(t('حدث خطأ', 'An error occurred'));
            }
        } else {
    ...
    const deleteFeaturedSubject = async (id: string) => {
        const { error } = await supabase.from('home_featured_subjects').delete().eq('id', id);
        if (!error) {
    ...
    const moveLesson = async (item: any, direction: 'up' | 'down') => {
        ...
        await supabase.from('home_featured_lessons').update({ home_order: newIndex }).eq('id', item.id);
        await supabase.from('home_featured_lessons').update({ home_order: currentIndex }).eq('id', otherItem.id);
    ...
    const addFeaturedLesson = async () => {
        ...
        const maxOrder = featuredLessons.length > 0
            ? Math.max(...featuredLessons.map(l => l.home_order)) + 1
            : 0;

        const { error } = await supabase
            .from('home_featured_lessons')
            .insert({ lesson_id: selectedItemId, home_order: maxOrder } as any);

        if (error) {
            if (error.code === '23505') {
                toast.error(t('هذا الدرس مضاف مسبقاً', 'Lesson already featured'));
            } else {
                toast.error(t('حدث خطأ', 'An error occurred'));
            }
        } else {
    ...
    const deleteFeaturedLesson = async (id: string) => {
        const { error } = await supabase.from('home_featured_lessons').delete().eq('id', id);
        if (!error) {
            toast.success(t('تم الحذف', 'Deleted'));
            fetchFeaturedLessons();
        }
                            ...
                            <div className="bg-background border border-border rounded-lg divide-y divide-border">
                                {featuredSubjects.map((item, index) => (
                                    <div key={item.id} className="flex items-center gap-4 p-4">
                                        ...
                                        {/* Info */}
                                        <div className="flex-1">
                                            <p className="font-medium">{item.subject?.title_ar}</p>
                                            <p className="text-xs text-muted-foreground">
                                                {item.subject?.level?.title_ar} • {item.teaser_ar || t('لا يوجد وصف', 'No teaser')}
                                            </p>
                                        </div>
                                        ...
                                        <Button variant="ghost" size="sm" onClick={() => deleteFeaturedSubject(item.id)}>
                            ...
                            <div className="bg-background border border-border rounded-lg divide-y divide-border">
                                {featuredLessons.map((item, index) => (
                                    <div key={item.id} className="flex items-center gap-4 p-4">
                                        ...
                                        <Button variant="ghost" size="sm" onClick={() => deleteFeaturedLesson(item.id)}>
                                            <Trash2 className="w-4 h-4 text-destructive" />
                                        </Button>
                                        <div className="flex items-center gap-2">
                                            {item.is_visible ? (
                            ...
                            >
                                ...
                                {activeTab === 'subjects' && allSubjects.map(subject => (
                                    <option key={subject.id} value={subject.id}>
                                        {subject.title_ar} - {(subject as any).level?.title_ar}
                                    </option>
                                ))}
