...
import { useSubjects, useStudentProgress } from '@/hooks/useQueryHooks';
...
export default function MySubjects() {
    ...
    const { data: progressData = [], isLoading: progressLoading } = useStudentProgress(profile?.id);

    const loading = subjectsLoading || progressLoading;
    const ChevronIcon = direction === 'rtl' ? ChevronLeft : ChevronRight;

    const { subjects, stages } = useMemo(() => {
        const completedLessonIds = new Set(
            (progressData as any[]).filter(p => p.completed_at).map(p => p.lesson_id)
        );

        const mapped = (subjectsData as any[]).map(s => {
        ...
        // Group by stage
        const stageMap = new Map<string, { stage: Stage; subjects: typeof mapped }>();
        mapped.forEach(s => {
            const stageId = s.stage_id;
            if (!stageMap.has(stageId)) {
                stageMap.set(stageId, { stage: s.stage!, subjects: [] });
            }
            stageMap.get(stageId)!.subjects.push(s);
        });

        return { subjects: mapped, stages: stageMap };
    }, [subjectsData, progressData]);
