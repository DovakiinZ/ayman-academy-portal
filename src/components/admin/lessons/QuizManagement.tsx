...
import { Lesson, LessonQuiz, LessonQuizQuestion } from '@/types/database';
    ...
    const fetchQuiz = async () => {
        try {
            ...
            const { data: quizData, error: quizError } = await (supabase
                .from('lesson_quizzes' as any) as any)
                .select('*')
            ...
            if (quizData) {
                ...
                const { data: qData, error: qError } = await (supabase
                    .from('lesson_quiz_questions' as any) as any)
                    .select('*')
...
function QuestionCard({ 
    ...
    index: number,
    onUpdate: (updates: Partial<LessonQuizQuestion>) => void,
    onDelete: () => void
}) {
    const { t } = useLanguage();
    ...
    const handleOptionChange = (optIndex: number, newValue: string) => {
        ...
        const updates: Partial<LessonQuizQuestion> = { options: newOptions };
